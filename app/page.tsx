...
import { Mail, Lock, ArrowRight, Wallet, ShieldCheck, LockKeyhole } from 'lucide-react';
...
export default function LoginPage() {
  ...
  return (
    ...
    <main className="min-h-screen flex flex-col relative overflow-hidden bg-background font-sans">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[400px] bg-brand/5 blur-[120px] rounded-full pointer-events-none" aria-hidden="true" />
      ...
      <div className="flex-1 flex flex-col items-center justify-center w-full px-6 py-12 z-10">
        ...
        <section className="bg-surface w-full max-w-[420px] mx-auto rounded-2xl p-6 md:p-8 shadow-2xl border border-outline/30">
          <div className="mb-8">
            <h2 className="text-xl md:text-2xl font-semibold mb-1 text-foreground">Bem-vindo de volta</h2>
          ...
          <form className="space-y-5" onSubmit={handleLogin}>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <a href="#" className="text-brand hover:text-brand-hover text-xs font-medium transition-colors focus:outline-none focus:underline rounded-sm focus:ring-2 focus:ring-brand focus:ring-offset-2 focus:ring-offset-surface">
                  Esqueceu a senha?
                </a>
              </div>
              <div className="relative">
              ...
              {error && (
                  <p className="text-alert text-sm font-medium" role="alert">Credenciais inválidas. Use admin@vault.com / admin123</p>
              )}
          ...
          <div className="mt-8 pt-6 border-t border-outline/50 text-center md:hidden">
            <p className="text-sm text-muted">
              ...
              <a href="#" className="text-brand hover:text-brand-hover font-medium focus:outline-none focus:underline">
                Criar agora
              </a>
            </p>
          </div>
      ...
      <footer className="w-full p-6 text-xs text-muted font-medium uppercase tracking-wider z-10 bg-background/50 backdrop-blur-sm">
        ...
        <div className="flex justify-between w-full max-w-[420px] mx-auto md:hidden">
          <div className="flex items-center gap-2">
            ...
            <span className="text-left leading-tight">Segurança<br/>Bancária</span>
          </div>
          <div className="flex items-center gap-2">
            <LockKeyhole className="w-4 h-4" aria-hidden="true" />
            <span className="text-left leading-tight">Criptografia Ponta-a-<br/>Ponta</span>
          </div>
        </div>

        {/* Desktop Footer */}
        <div className="hidden md:flex justify-center items-center">
          SEGURANÇA E PRIVACIDADE • &copy; {new Date().getFullYear()} THE DIGITAL VAULT
        </div>
      </footer>
    </main>
  );
